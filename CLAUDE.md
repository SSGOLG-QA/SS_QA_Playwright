...
- **대회 > 대회관리**: `locators/admin-tournament.md` + `runTournament`/`runTournamentPopups` + `Admin/tournament.spec.ts` — ✨ **2026-06 대회모드 TC 기준 재작성 + 팝업 심화 + 불변식 실데이터 검증 + td18 라이브(2026-07-16, PASS 17/FAIL 0/SKIP 2, 차이 9, TOURN-01~16)**. URL `/club/page/tournament?cp=1`.
  ...
  - ✨ **영어 i18n E2E(2026-07-29)**: `Admin/tournament-lang-e2e.spec.ts` — 버튼 클릭 → **랜딩 화면**(인페이지 모달 5종 + **새 탭 2종** TournamentScore/TournamentRank) 언어 검증(td18, PASS 7/FAIL 1). 구조적 chrome만 스캔(tbody/입력값/고유명사 배제 → 오탐 제거). 🔴 **실제 i18n 결함 1건**: 대회 등록 모달 라벨 `태블릿 리더보드 노출` 영어 미번역(옵션 노출/미노출은 번역됨). ⚠ 언어 독립 주의: tableBox 필터·행 상태·모달 닫기는 한글 텍스트 의존 금지(영어 전환 시 실패) — 테이블 행 존재·사전 캡처 행·클래스 기반 닫기(`Modal.closeNonDestructive`) 사용.
