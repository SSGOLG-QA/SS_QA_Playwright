...
### 병렬 실행 (계정 풀, 선구현 — 기본은 단일 계정 직렬)
- **기본**: `ACCOUNT_COUNT` 미설정=1 → `fullyParallel:false`, `workers:1`(단일 계정 직렬, 현 동작 동일·안전). 같은 계정 동시 로그인은 서버가 강제 로그아웃하므로 단일 계정으로는 병렬 금지.
- **계정 풀 확보 시**: 테스트 계정 N개를 인증 → `$env:ACCOUNT_COUNT="N"; npm run auth`(N회 순차 수동 로그인, 계정마다 **다른 계정**으로. 저장: 계정0=`admin.json`, 1..=`admin-<i>.json`). 이후 `$env:ACCOUNT_COUNT="N"; npm run test:admin`(또는 test:lang) → `fullyParallel:true`, `workers:N`, 워커별 distinct 계정(`fixtures.ts`의 `storageState`가 `parallelIndex % N`로 분배)으로 N병렬.
- ⚠ `test:full`·`test:ia`는 `--workers=1` 하드코딩(파괴/순서 민감 → 직렬 유지). all-suite는 단일 test라 병렬 무관(개별 스펙 병렬로 대체). 파괴 테스트(Playwright_New·lang-toast)는 공유 데이터 변경 → 병렬 제외.
...
- **대회 > 대회관리**: `locators/admin-tournament.md` + `runTournament`/`runTournamentPopups` + `Admin/tournament.spec.ts` — ✨ **2026-06 대회모드 TC 기준 재작성 + 팝업 심화 + 불변식 실데이터 검증 + td18 라이브(2026-07-16, PASS 17/FAIL 0/SKIP 2, 차이 9, TOURN-01~16)**. URL `/club/page/tournament?cp=1`.
  - **TC 출처**: 드라이브 > 01.TC > **2026-06 대회모드**(id `1o5gXwl7Lt5RpL3c0GVEyYZSG6liO7A63PnfHqacNQZY`), 시트 **`대회모드_Cloud`**(531 TC). tcRef=`대회_대회관리_{No.}`. **TC host=td18** → `SUBDOMAIN=td18`로 검증(환경 전환 참조).
  - **메인 화면**(No.1~61): TOURN-01~10 — 상단문구·웹뷰 URL 2종·검색·[신규 등록]·대회리스트 15컬럼·접속 인증키 2종·접속키 형식·행 액션.
  - **읽기전용 팝업 심화**(비파괴, `runTournamentPopups`): ✅ 인페이지 모달 4종 — 대회 등록(TOURN-13, 순위산정 라디오 6종·이벤트), 참가자 등록(14, 리스트 8컬럼·액션), 조편성(15, 라운드칩·자동배치·티생성), 그룹편집(16). ✅ 새 탭 2종 — 스코어(11/11-1, 구조+`groupScoreInvariants` 합계=전반+후반·합계−오버=Par), 결과집계/출력(12/12-1, 구조+`rankOrder`/`teamAvg` 순위 연속·Team Average=mean). **불변식은 상태 완료/진행중 대회(`sRow`) 대상 실행·통과(계산 결함 0)**. 편집·저장·삭제·엑셀 업로드·시상편집 팝업(No.298~418)은 데이터 변경 → 🔴 노출/구조만.
  - ⚠ **컬럼-td 정렬**: 2R+ 행은 rowspan으로 td 축소 → 행액션은 **첫 행(row0, 대회명 노출 행)** 기준. `colIdx` 정확일치 우선(`참가자`⊂`참가자수` 충돌 방지). td: 참가자6/조편성7/그룹편집8/스코어9/결과집계10.
  - ⚠ **TOURN-16 자동화 불안정**: 그룹편집 [설정] 모달이 순간 노출 후 즉시 닫힘 → 열린 순간 원자적 DOM 스냅샷 캡처 후 검증, 실패 시 **SKIP**(가짜 FAIL 방지, 구조는 프로브로 확인).
  - ⚠️ **기획-구현 차이 9건(td18 확인, `diff`·QA 확인 요망)**: 검색 ph `대회명`→`검색어 입력` / 등록 `대회 등록`→`신규 등록` / 접속 `ID`→`인증키` / 접속키 `16진수3`→`영숫자5`(`260716-W1O86`) / `온라인 참가자 접수` 컬럼 미구현 / **그룹편집 트리거 `[등록]/[보기]`→`[설정]`·명칭 `그룹관리`→`그룹편집 / 핸디관리`** / **메모 `600자`→`200자`** / 참가자 컬럼 `No·휴대폰번호`→`순서·연락처`. 검증은 **구현 기준** PASS. ✅ 리더보드(`/club/TournamentLeaderBoard`) ≠ 관리자(`/club/TournamentAdminView`) URL 별도 경로 해소.
  - ⛔ 리더보드·관리자 웹뷰(No.420~531, `smartscore.kr` 별도 웹앱) = 범위 외(랜딩만) → `diff`. 행 액션·접속 인증키·상태·스코어/순위 불변식은 데이터 의존(0건/미노출 SKIP).
  - ✨ **다국어 i18n E2E(2026-07-30)**: `Admin/tournament-lang-e2e.spec.ts` — 버튼 클릭 → **랜딩 8종**(메인 + 대회등록·참가자·조편성·그룹편집 모달 + **접속키 복사 안내 팝업** + 스코어·결과집계 **새 탭**) **7개국 per-element** 검증(대회모드_Cloud TC 참조, `LANGS` env로 언어 선택). 표준 langCheck과 동일 수준: KO baseline↔FG 요소별 대조 → PASS(번역값)/한글노출/혼재/미번역, tcRef·zone 기록. 언어별 리포트 `tournament-lang-<언어>_report`. 검증(td18): 영어 82·베트남어 79·인니 — **결함 0**.
    - **스캔 범위**: 구조적 chrome(제목·버튼·컬럼·라벨·placeholder) + **안내문구 문단**(`capInfoGuide`/GSEL — 메인 상단 소개문구·모달 설명) + **행 버튼명·상태값**(`capRowSystem` — 진행전/진행중/완료, tbody지만 시스템 텍스트). 데이터(tbody 데이터셀·입력값·고유명사 클럽/대회명) 배제 → 오탐 제거.
    - ⚠ **언어 독립 필수**(영어 전환 시 한글 매칭 실패): tableBox=테이블 행 존재 필터 / 행 상태 필터 금지(사전 캡처 행) / 모달 닫기·언어 전환 전 `cleanOverlays`(취소·[확인]·닫기 dismiss, class 기반). 언어 전환은 `switchLangRobust`(오버레이 정리+재시도), 언어마다 대회관리 재진입.
    - ⚠ **랜딩 오픈 안정화(핵심 교훈)**: ① **새 탭(스코어/결과집계)은 모달 개폐 누적 전 메인 직후 먼저 열기**(누적 상태서 window.open 차단됨 — 순서 의존) ② **접속키 [복사] 안내 모달은 clipboard 권한 필요**(`test.use({permissions:['clipboard-read','clipboard-write']})` — 미부여 시 writeText 실패로 모달 미노출, 근본원인) ③ **그룹편집 [설정] 모달은 순간 노출 후 자동 닫힘** → 대기 없이 100ms 폴링 즉시 캡처 ④ 일부 팝업 KO baseline 간헐 0 → **FG 단독 판정 폴백**(KO 없어도 FG 한글잔존 검사).
    - ℹ️ 이전 `태블릿 리더보드 노출` 미번역 결함은 **해당 필드가 대회 등록 모달에서 제거되어 해소**(TC R64 부재 확인).
    - ⚠ **td18 세션 제약**: 공유 QA 계정 동시 사용으로 **재로그인 1회당 1런만 생존**. 단일 컨텍스트 다국어 순회는 3번째 언어부터 전환 막힘 → **언어별 개별 실행(각 `npm run auth` + `LANGS=<언어>`)이 안정적**.
  - ✨ **화면 구성요소 분석**: `locators/admin-tournament-components.md` — 텍스트/버튼/placeholder/컬럼/라디오/체크 전수(메인+팝업 6종+서브모달). 라이브 전수(✅) vs 트리거+TC 기준(🟡) 커버리지 명시. 정정: 메모 placeholder `최대 600자`(TC 일치, 이전 200자 오독 철회)·태블릿 리더보드 옵션 `노출/미노출`.
...
- ✨ **언어 검증(전 메뉴 다항목 UI 표기 검증)**: `lib/langCheck.ts` + `Admin/lang-check-all.spec.ts`(전체) / `Admin/lang-check.spec.ts`(PoC) — **검증 완료(2026-06-09)**. **공식 지원 언어 7개**(한국어=baseline 제외: English/Tiếng Việt/ภาษาไทย/繁體中文/简体中文/日本語/Bahasa Indonesia) 전 메뉴에서 **정적 UI 표기 결함 전수 검출**. **언어별 리포트 파일 분리**(`reports/lang-check-<언어>_report_*.xlsx`, 7개). 전체 실행 ~26분.
  ...
  - ✨ **진입 cascade 차단 + 외국어 백지 검출(2026-06-19)**: 다국어 검증에서 ①구현된 메뉴가 'SKIP 미구현/범위제외'로 오분류 ②외국어 전환 후 백지 화면이 PASS로 묻히던 2대 결함 수정(`lib/langCheck.ts`). **근본원인**: `switchLanguage(KOREAN)` 원복 반환값 무시 → 안 닫히는 팝업(예 홀맵 [구역 관리] 맵에디터)이 언어 트리거 차단 → SNB 외국어 잔존 → 다음 메뉴 `navigateMenu(한국어명)` 매칭 실패 → 연쇄 SKIP. + `isContentBlank`를 한국어 상태에서만 검사 → 외국어 백지 누락. **수정 5종**: `ensureKorean`(3단 복구: 오버레이 닫고 전환→DOM클릭 z-index우회→**홈 리셋** 후 재시도, 모든 원복 지점 적용) / `snbLinkExists`로 미구현(링크부재) vs 진입실패(링크有·네비실패) 구분 / `failForeignBlank`로 **언어 전환 후(외국어 모드)** 본문 백지 검출 — `isContentBlank`는 **폴링 positive-wait 6초**(즉시 스냅샷 금지). ⚠ 진입(한국어) 시점 blank 검사는 **제거**(2026-06-22): navigate 직후 최이른 시점이라 데이터 로딩 화면(캐디 리스트·고객 평가 등)이 폴링으로도 false-blank 오탐(영어 24건→continue로 스캔까지 누락). 외국어 백지는 captureSlots·전환 뒤(콘텐츠 로드 완료 시점)라 오탐 없음 + 진짜 지속 백지(6s+ 미렌더)만 FAIL / `enterMenuChecked`+`recordEntryFailure`로 사유 분리(결함=FAIL / 정당 미구현=SKIP). 3개 순회(All/Unified/Dynamic) 공통. **풀 회귀 검증(7개 언어, 2026-06-22 클린)**: 진입 false-blank **0**, 외국어 백지 **0**, cascade(진입실패/원복실패) **0**, SKIP 56 전부 `미구현(SNB 링크 없음)` 정당(false 미구현 0). 언어당 PASS ~2,690 / FAIL ~29(영/베/번/간/인니 29·일 30·태 51=한글노출+영어노출) — FAIL 전부 실제 번역 결함(한글노출 203·타 언어 22·혼재 1). ⚠ **이전 '제품결함 2건(홀맵 미리보기/VI·내장통계/TH 외국어 백지)'은 철회** — 클린 run에서 정상 렌더(각 22·21 PASS). 06-19 백지는 cascade 잔존 + 즉시 스냅샷이 겹친 **테스트 아티팩트**였고, cascade 복구+폴링 검사로 해소. 진단 프로브 `Admin/_probe-holemap-preview.spec.ts`·`Admin/_probe-blank-timing.spec.ts`.
...
- **P4-2 API 계약 검증 심화**: `lib/apiContract.ts` 강화 + `Admin/api-contract.spec.ts`(9화면 **단일 test 순회**). **핵심**: ① **발견된 키 없이도 일반 불변식 수행** — 상태 2xx·CT JSON·JSON 파싱/최상위타입(object|array)·**에러 envelope 미포함**(`success:false`/`error`/`code≥400`/`status≥400`). ② **단일 URL 키워드 의존 제거** — `startCapture`가 페이지의 **모든 데이터 API(GET/POST, 정적자산 제외) 수집**, `pick()`이 키워드+JSON+2xx > 키워드+2xx > (fallbackAny) JSON+2xx > 임의 2xx로 선택. 9화면: 내장현황·전체라운드·주문내역·계정리스트·대회관리·캐디리스트·후기리스트·핀포지션·배토기록.
  - ⚠️ **단일 test 순회 필수(2026-08-05)**: `admin` 픽스처가 test 스코프 → test마다 openAdmin(재로그인). 화면당 개별 test면 **같은 계정 5~6 로그인 후 서버 강제 로그아웃**(로그인 폭주). all-suite처럼 **한 test에서 1회 로그인으로 전 화면 순회**로 리팩터. 세션은 **재로그인 1회당 1런만 생존**(td17·td18 공통, 공유 QA 계정).
  - ⚠️ **아키텍처 발견(2026-08-05 td17 라이브, PASS 9/SKIP 9)**: 어드민 SPA는 **부팅 시 데이터 프리페치/클라이언트 캐시** → 메뉴 이동은 순수 클라이언트 렌더라 **진입 시 데이터 API 네트워크 미발생**(전 9화면 '데이터 API 0건'). 하네스는 정상(가짜 FAIL 없이 SKIP). **후속**: 실제 API 캡처하려면 ① openAdmin(부팅) 시점 startCapture로 프리페치 수집, 또는 ② 화면별 [조회]/[검색]·`admin.reload()`로 refetch 유발 후 캡처(reload=비파괴). expectedKeys/countPath 채우기는 그 다음.
- **P4-3 UI 구조 드리프트 검출**: `Admin/ui-baseline.spec.ts`. 전 메뉴(MENU_LIST) 구조 chrome(컬럼헤더·버튼·탭·안내문구)을 admin.evaluate로 스캔 → committed baseline `baselines/ui-structure.<sub>.json`과 대조. **제거=check FAIL(회귀 신호)·추가=diff(기획-구현 차이 시트)·화면소실=FAIL·신규화면=diff**. 산출 `reports/ui-drift_<ts>.md` + `ui-baseline_report_*.xlsx`. env `UI_BASELINE=capture`(baseline 생성/갱신) vs 기본 diff(대조, baseline 부재 시 최초 자동 캡처). **기존 `driftDiff.ts`와 상보적**(driftDiff=리포트 간 FAIL 증감→기대값 구값일 때만 노출 / 본 스펙=구조 직접 대조→기대값 미수정 상태서도 제거 즉시 포착). 06-05/08/16 반복된 수동 드리프트 추적(all-suite→FAIL 산발→프로브→AS-IS)을 단일 스캔으로 대체. 비파괴(스캔만). ✅ **baseline 캡처 완료(2026-08-05 td17)**: `baselines/ui-structure.td17.json` — 44화면(헤더 393·버튼 238), 단일 test 1회 로그인 2분. 이후 diff 모드로 구조 드리프트 자동 검출.
