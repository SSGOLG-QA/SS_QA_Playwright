...
- **대회 > 대회관리**: `locators/admin-tournament.md` + `runTournament`/`runTournamentPopups` + `Admin/tournament.spec.ts` — ✨ **2026-06 대회모드 TC 기준 재작성 + 팝업 심화 + td18 라이브 검증(2026-07-16, PASS 15/FAIL 0/SKIP 4, 차이 9, TOURN-01~16)**. URL `/club/page/tournament?cp=1`.
  ...
  - **읽기전용 팝업 심화**(비파괴, `runTournamentPopups`): ✅ 인페이지 모달 4종 — 대회 등록(TOURN-13, 순위산정 라디오 6종·이벤트), 참가자 등록(14, 리스트 8컬럼·액션), 조편성(15, 라운드칩·자동배치·티생성), 그룹편집(16). ✅ 새 탭 2종 — 스코어(11, 구조+`groupScoreInvariants` 합계=전반+후반), 결과집계/출력(12, 구조+`rankOrder`/`teamAvg` 불변식). 편집·저장·삭제·엑셀 업로드·시상편집 팝업(No.298~418)은 데이터 변경 → 🔴 노출/구조만.
