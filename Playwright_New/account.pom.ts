...
import { MENU, SCREEN, TIMEOUT } from './account.data';
...
export class AccountBasePage {
  ...
  async renderedRowCount(): Promise<number> {
    const empty = await this.rows.filter({ hasText: /내역이 없|데이터가 없|조회된|계정이 없/ }).count().catch(() => 0);
    return empty > 0 ? 0 : await this.rows.count();
  }
}

export class AccountListPage extends AccountBasePage {
  async open() { await super.open(SCREEN.list.sub); }
  searchInput(): Locator { return this.page.getByPlaceholder(SCREEN.list.searchPh); }
  rowActionBtn(): Locator { return this.page.getByRole('button', { name: SCREEN.list.rowActionBtn }); }
}
export class AccountPermissionPage extends AccountBasePage {
  async open() { await super.open(SCREEN.permission.sub); }
}
