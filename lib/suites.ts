...
const NOTICE = {
  ...
  stats: '스마트스코어 회원이 내장하여 라운드기록을 전송받는 경우, 골프장에서는 해당 회원의 정보를 활용하여 타겟마케팅이 가능합니다. 스마트스코어 회원이 라운드기록을 전송받은 데이타에 근거하여 골프장 내장객의 성격과 추이를 확인할 수 있습니다.',
...
export async function runCaddieList(admin: Page) {
  ...
  for (const [i, c] of ['No', '성명', '성별', '휴대폰', '카트번호', '태블릿 No.', '배터리', '라운드기록', '회원추천', '그늘집주문'].entries())
    await check(admin, { path: `${P} > 테이블`, tcRef: `${R}_5`, tcId: `CADL-05-${i + 1}`, desc: `컬럼 '${c}' 노출`, expected: `'${c}'`, failMsg: '컬럼 미노출' },
      async () => { await expect(admin.getByRole('columnheader', { name: c, exact: false }).first()).toBeVisible(); });
  await check(admin, { path: `${P} > 액션`, tcRef: `${R}_6`, tcId: 'CADL-06', desc: '[관제적용] 버튼 노출(클릭 미수행·비파괴)', expected: '관제적용', failMsg: '관제적용 미노출' },
    async () => { await expect(admin.getByRole('button', { name: '관제적용' })).toBeVisible(); });
  await runCommonActions(admin, P, R);
}
...
export async function runBetoRecord(admin: Page) {
  ...
  for (const [i, c] of ['No.', '캐디', '시작시간', '종료시간', '작업 경로'].entries())
    await check(admin, { path: `${P} > 테이블`, tcRef: `${R}_3`, tcId: `BREC-03-${i + 1}`, desc: `컬럼 '${c}' 노출`, expected: `'${c}'`, failMsg: '컬럼 미노출' },
      async () => { await expect(admin.getByRole('columnheader', { name: c, exact: false }).first()).toBeVisible(); });
  await check(admin, { path: `${P} > 행`, tcRef: `${R}_4`, tcId: 'BREC-04', desc: '행 [보기](작업 경로) 버튼 노출(비파괴)', expected: '보기', failMsg: '보기 버튼 미노출' },
    async () => { await expect(admin.getByRole('button', { name: '보기', exact: true }).first()).toBeVisible(); });
  await runCommonActions(admin, P, R);
}
...
export async function runHolemapZone(admin: Page) {
  ...
  await check(admin, { path: `${P} > 필터`, tcRef: `${R}_2`, tcId: 'HMZ-02', desc: '필터(코스/홀 vue-select) + [초기화]/[적용]/[구역관리](비파괴)', expected: '필터+버튼', failMsg: '필터/버튼 미노출' },
    async () => { expect(await admin.locator('.vs__dropdown-toggle').count()).toBeGreaterThanOrEqual(1); await expect(admin.getByRole('button', { name: '적용', exact: true }).first()).toBeVisible(); await expect(admin.getByRole('button', { name: '구역관리' }).first()).toBeVisible(); });
...
export async function runHolemapTeeshot(admin: Page) {
  ...
  await check(admin, { path: `${P} > 홀별 거리`, tcRef: `${R}_3`, tcId: 'HMTS-03', desc: '홀별 거리 입력 + 사용 checkbox 노출(≥1, PAR3 비활성)', expected: 'input+checkbox', failMsg: '입력 미노출' },
    async () => { expect(await admin.locator('.contents-box input[type="checkbox"]').count()).toBeGreaterThanOrEqual(1); await expect(admin.getByPlaceholder('미입력').first()).toBeVisible(); });
  await check(admin, { path: `${P} > 버튼`, tcRef: `${R}_4`, tcId: 'HMTS-04', desc: '[초기화]/[저장] 버튼 노출(비파괴)', expected: '초기화/저장', failMsg: '버튼 미노출' },
    async () => { await expect(admin.getByRole('button', { name: '저장', exact: true })).toBeVisible(); await expect(admin.getByRole('button', { name: '초기화' })).toBeVisible(); });
  await runCommonActions(admin, P, R);
}
...
export async function runPinPosition(admin: Page) {
  ...
  for (const [i, c] of ['코스명', '홀', 'PAR', '야디지', '그린', '핀포지션', '선택'].entries())
...
export async function runCourseAnalysis(admin: Page) {
  ...
  for (const [i, c] of ['홀', '스코어', '퍼팅수', '페어웨이안착률', '그린적중률'].entries())
...
export async function runReviewStats(admin: Page) {
  ...
  await check(admin, { path: `${P} > 검색`, tcRef: `${R}_2`, tcId: 'RVS-02', desc: '기간 버튼 + datepicker(≥2) + [조회]/[내보내기](비파괴)', expected: 'datepicker≥2', failMsg: '검색 미노출' },
    async () => { expect(await admin.locator('.datepicker-input').count()).toBeGreaterThanOrEqual(2); await expect(admin.getByRole('button', { name: '조회' })).toBeVisible(); });
  for (const [i, c] of ['순서', '날짜', '등록후기수', '전체', '코스', '그린', '서비스', '진행', '식음료'].entries())
...
export async function runFnbOrderHistory(admin: Page) {
  ...
  await check(admin, { path: `${P} > 검색`, tcRef: `${R}_6`, tcId: 'FNBORD-06', desc: '[검색]/[초기화] 버튼 노출', expected: '검색/초기화', failMsg: '버튼 미노출' },
    async () => { await expect(admin.getByRole('button', { name: '검색', exact: true }).first()).toBeVisible(); await expect(admin.getByRole('button', { name: '초기화', exact: true }).first()).toBeVisible(); });
...
const IA_TREE: { menu: string; subs: string[] }[] = [
  ...
  { menu: '관제 관리', subs: ['관제팝업', '메시지 기록 조회', '라이브채팅 공지 조회', '아이콘 관리', '카트이동경로 확인'] },
